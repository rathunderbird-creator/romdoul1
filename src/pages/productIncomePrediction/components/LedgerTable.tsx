...
const COLUMNS = ['day', 'units', 'orders', 'pending', 'revenue', 'cogs', 'grossProfit'] as const;
    ...
    const renderRow = (day: LedgerDay, idx: number) => {
        ...
        return (
            ...
            >
                ...
                <td style={{ ...widthStyle('units'), textAlign: 'right', fontSize: 12, fontWeight: 600, color: day.units > 0 ? 'var(--color-text-main)' : 'var(--color-text-secondary)', padding: 8 }}>{day.units > 0 ? day.units : '-'}</td>
                ...
                <td style={{ ...widthStyle('pending'), textAlign: 'right', fontSize: 12, padding: '8px 12px', color: day.pendingUnits > 0 ? '#F59E0B' : 'var(--color-text-secondary)', fontWeight: 600, whiteSpace: 'nowrap' }}>{day.pendingUnits > 0 ? day.pendingUnits : '-'}</td>
            ...
            <table className="spreadsheet-table" style={{ borderCollapse: 'separate', borderSpacing: 0, tableLayout: 'fixed', width: totalWidth, minWidth: '100%' }}>
                <thead>
                    <tr style={{ background: 'var(--color-surface)' }}>
                        {header('day', t('productPrediction.columns.day'))}
                ...
                <tfoot>
                    <tr style={{ borderTop: '2px solid var(--color-border)' }}>
                        {footCell('day', t('productPrediction.totals'), undefined, 'left')}
                        {footCell('units', totals.units || '-')}
                        {footCell('orders', totals.orders || '-')}
                        {footCell('pending', totals.pendingUnits || '-', '#F59E0B')}
                    ...
                    <tr>
                        <td colSpan={6} className="pip-sticky-first" style={{ fontWeight: 800, fontSize: 14, padding: 14, color: netColor, background: netBg, borderBottomLeftRadius: 16 }}>
                            {totals.grossProfit >= 0 ? '🟢' : '🔴'} {t('productPrediction.grossProfitTotal')}
                        </td>
